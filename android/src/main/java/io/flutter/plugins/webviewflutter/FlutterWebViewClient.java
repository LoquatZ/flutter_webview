...
class FlutterWebViewClient {
    ...
    private void shouldInterceptRequest(WebView view, final String url) {
        //因为shouldInterceptRequest回调于子线程，Flutter不支持异步线程回调
        //故切换至UI(主线程)调用
        SyncExecutor.getInstance().invokeMethod(methodChannel, "shouldInterceptRequest", url);
    }
    ...
    private WebViewClient internalCreateWebViewClient() {
        return new WebViewClient() {
            ...
            public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
                FlutterWebViewClient.this.shouldInterceptRequest(view, url);
                return super.shouldInterceptRequest(view, url);
            }

            //为兼容 API < 21 将 shouldInterceptRequest切换 至 shouldInterceptRequest(WebView view, String url)
            @SuppressLint("NewApi")
            @Override
